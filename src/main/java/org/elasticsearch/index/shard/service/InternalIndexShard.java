...
public class InternalIndexShard extends AbstractIndexShardComponent implements IndexShard {
    ...
    @Override
    public Engine.Create prepareCreate(SourceToParse source, long version, VersionType versionType, long sequenceId, Engine.Operation.Origin origin, boolean canHaveDuplicates, boolean autoGeneratedId) throws ElasticsearchException {
        ...
        return new Engine.Create(docMapper.v1(), docMapper.v1().uidMapper().term(doc.uid().stringValue()), doc, version, versionType, sequenceId, origin, startTime, state != IndexShardState.STARTED || canHaveDuplicates, autoGeneratedId);
    }
    ...
    @Override
    public Engine.Index prepareIndex(SourceToParse source, long version, VersionType versionType, long sequenceId, Engine.Operation.Origin origin, boolean canHaveDuplicates) throws ElasticsearchException {
        long startTime = System.nanoTime();
        Tuple<DocumentMapper, Boolean> docMapper = mapperService.documentMapperWithAutoCreate(source.type());
        ParsedDocument doc = docMapper.v1().parse(source).setMappingsModified(docMapper);
        return new Engine.Index(docMapper.v1(), docMapper.v1().uidMapper().term(doc.uid().stringValue()), doc, version, versionType, sequenceId,
                                origin, startTime, state != IndexShardState.STARTED || canHaveDuplicates);
    }
            ...
            switch (operation.opType()) {
                case CREATE:
                    ...
                    Engine.Create engineCreate = prepareCreate(
                            ...
                            create.version(), create.versionType().versionTypeForReplicationAndRecovery(), create.sequenceId(), Engine.Operation.Origin.RECOVERY, true, false);
                ...
                case SAVE:
                    ...
                    Engine.Index engineIndex = prepareIndex(source(index.source()).type(index.type()).id(index.id())
                                    .routing(index.routing()).parent(index.parent()).timestamp(index.timestamp()).ttl(index.ttl()),
                                                            index.version(), index.versionType().versionTypeForReplicationAndRecovery(),
                                                            index.sequenceId(), Engine.Operation.Origin.RECOVERY, true);
